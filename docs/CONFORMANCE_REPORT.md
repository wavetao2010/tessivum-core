...
**Repository source release:** `0.1.4` · all runtime crates remain `publish = false`; this is not a crates.io package release.
...
`0.1.1` reran all 111 Rust workspace tests and refreshed
[`BENCHMARK_RESULTS.json`](BENCHMARK_RESULTS.json) on 2026-08-24. `0.1.2`
added the MIT license; `0.1.3` accepts bounded startup log frames before the
Legacy Node host's ready frame; `0.1.4` loads the compiled pinned Cordis modules
in packaged deployments. These releases do not change the recorded TypeScript oracle baseline.
